...
class JSON_API_Core_Controller {
    public function get_config() {
        ...
        $sidemenus         = $this->get_sidemenu_index();
        $category_item_num = 5;

        return array(
            'api_version'       => NUTNOTE_API_VERSION,
            'category_item_num' => $category_item_num,
			'top_item_num' 		=> NUTNOTE_TOP_ITEM_NUM,
			'app_title'         => 'NutNote',
    ...
    public function get_sidemenu_index() {
        $locations      = get_registered_nav_menus();
        $menus          = wp_get_nav_menus();
        $menu_locations = get_nav_menu_locations();

        $location_id = 'nutnote_navigation';
        if ( ! isset( $menu_locations[ $location_id ] ) ) {
            $location_id = 'header_navigation';
        }
        $result = array();
        if ( isset( $menu_locations[ $location_id ] ) ) {
            foreach ( $menus as $menu ) {
                ...
                if ( $menu->term_id == $menu_locations[ $location_id ] ) {
                    ...
                    foreach ( $menu_items as $menu_item ) {
                        ...
                        $item['object_type'] = $menu_item->object;
    ...
    public function get_category_posts() {
        ...
        $index = $json_api->query->page || $json_api->query->index;
        if ( ! $index ) {
            $index = 0;
        }
        $posts = $json_api->introspector->get_posts( array(
